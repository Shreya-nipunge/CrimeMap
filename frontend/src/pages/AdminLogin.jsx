...
import { Mail, Lock, ShieldAlert, ArrowLeft, Building2, ChevronDown } from 'lucide-react';
...
export default function AdminLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.organization) {
      setError('Organization/Department is required');
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-blue-950 to-slate-900 flex items-center justify-center p-4 font-sans text-white relative overflow-hidden">
      ...
      <div className="w-full max-w-md relative z-10 transition-all duration-700 animate-in fade-in slide-in-from-bottom-4">
        ...
        >
          <ArrowLeft size={16} className="group-hover:-translate-x-1 transition-transform" /> Back to roles
        </button>
        ...
        <div className="bg-slate-900/60 backdrop-blur-xl border border-slate-700/50 rounded-2xl p-8 shadow-2xl relative overflow-hidden">
          ...
          <div className="flex flex-col items-center mb-8 relative z-10">
            ...
            <h2 className="text-3xl font-extrabold tracking-tight bg-gradient-to-r from-white to-slate-400 bg-clip-text text-transparent">Admin Portal</h2>
            <p className="text-blue-400/60 text-sm mt-2 font-medium tracking-wide text-center">Management & Data Entry</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4 relative z-10">
            <div>
              <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] mb-2 px-1">Admin Email</label>
              <div className="relative group">
                ...
                <input
                  ...
                  placeholder="admin@crimemap.gov"
                />
              </div>
            </div>
            ...
            <div>
              <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] mb-2 px-1">Administrative Role</label>
              <div className="relative group">
                <select
                  name="role"
                  value={formData.role}
                  onChange={handleChange}
                  className="w-full bg-slate-900/70 border border-slate-700 rounded-lg px-4 py-3 text-slate-200 focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/30 transition-all appearance-none cursor-pointer"
                >
                  <option>Police Admin</option>
                  <option>Data Analyst</option>
                  <option>System Admin</option>
                </select>
                <ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-500 pointer-events-none group-focus-within:text-blue-400 transition-colors" size={18} />
              </div>
            </div>

            <div>
              <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] mb-2 px-1">Organization / Department</label>
              <div className="relative group">
                <Building2 className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500 group-focus-within:text-blue-400 transition-colors" size={18} />
                <input
                  name="organization"
                  type="text"
                  required
                  ...
                  onChange={handleChange}
                  className="w-full bg-slate-900/70 border border-slate-700 rounded-lg pl-10 pr-4 py-3 text-slate-200 focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/30 transition-all placeholder:text-slate-600"
                  placeholder="e.g. Thane Police Dept"
                />
              </div>
            </div>
          ...
          <div className="mt-8 text-center relative z-10">
            <p className="text-slate-500 text-sm">
              New admin?{' '}
              <Link to="/register-admin" className="text-blue-400 hover:text-blue-300 transition-colors font-bold ml-1">
                Request Access
              </Link>
            </p>
          </div>
