...
import { User, Mail, Lock, ShieldAlert, ArrowLeft, Building2, ChevronDown } from 'lucide-react';
...
export default function AdminRegister() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const data = await registerAdmin(formData.email, formData.password, formData.fullName);
      ...
      <div className="w-full max-w-md relative z-10 transition-all duration-700 animate-in fade-in slide-in-from-bottom-4">
        ...
        <div className="bg-slate-900/60 backdrop-blur-xl border border-slate-700/50 rounded-2xl p-8 shadow-2xl relative overflow-hidden">
          ...
          <div className="flex flex-col items-center mb-8 relative z-10">
            ...
            <h2 className="text-3xl font-extrabold tracking-tight bg-gradient-to-r from-white to-slate-400 bg-clip-text text-transparent">Admin Registration</h2>
            <p className="text-blue-400/60 text-sm mt-2 font-medium tracking-wide">Request administrative credentials</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4 relative z-10">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] mb-2 px-1">Confirm</label>
                <div className="relative group">
                  <Lock className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500 group-focus-within:text-blue-400 transition-colors" size={18} />
                  <input
            ...
            <div>
              <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] mb-2 px-1">Organization</label>
              <div className="relative group">
          ...
          <div className="mt-8 text-center relative z-10">
            <p className="text-slate-500 text-sm">
              Back to{' '}
              <Link to="/login-admin" className="text-blue-400 hover:text-blue-300 transition-colors font-bold ml-1">
                Sign In
              </Link>
            </p>
          </div>
